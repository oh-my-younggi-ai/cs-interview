...
이 위키는 자매 스킬 `cs-interviewer`의 정답 기준으로도 쓰인다. 각 지식 페이지는 면접관이
...
- 대화 탐색과 추천에서는 한 번에 한 질문이나 제안만 하고 사용자 답변을 대신 만들지 않는다. 학습 중에는 현재 진단 질문을 `{pending_diagnostic_question}`으로 보존한다. 사용자가 그 질문의 의도, 용어, 조건을 묻거나 보충 질문을 하면 그것은 답변 시도로 처리하지 않는다. 물은 내용만 필요한 범위에서 답하고, 모범 답안, 채점 기준, 다음 항목을 덧붙이지 않은 채 원래 질문에 다시 답할 기회를 남긴다.
...
- 한 번의 학습 대화가 여러 독립 개념, 비교, 적용 문제를 포함하면 하나의 페이지에 강제로 합치지 않는다. 독립적으로 정의하고 면접 질문을 만들 수 있는 지식 단위마다 파일을 나누며, 비교나 적용 자체가 핵심이면 별도 페이지로 둔다.
- 여러 페이지의 공통 맥락은 관련 개념 링크와 필요한 비교 페이지로 연결한다. 같은 상세 설명을 여러 페이지에 반복하지 않는다.
...
- `{content_units}` - 승인된 학습 내용을 독립 개념, 비교, 적용 문제로 나눈 저장 단위. 각 단위는 제목, 파일명, 분야, 큰 토픽, 포함할 내용과 연결할 다른 단위를 가진다.
