...
- 새 토픽이나 개념의 첫 질문은 가장 필수적인 용어가 무엇인지 확인하는 정의 질문으로 시작한다. 정의가 확인되기 전에는 동작 원리, 비교, 예외, 적용 질문으로 넘어가지 않는다.
...
- `{core_terms}` - 현재 토픽이나 개념에서 세부 질문 전에 정의를 확인할 최소 핵심 용어
- `{current_core_term}` - 현재 정의를 확인하는 핵심 용어
- `{defined_core_terms}` - 이번 세션에서 사용자가 정의를 정확히 설명한 핵심 용어
- `{question_phase}` - 현재 개념의 질문 단계. `definition` | `detail` | `followup`
